...
#######################################
#### Data visualization side-track ####

# Underlying idea of using linear response shape is that there is an underlying non-linear resopnse function relating tree growth to moisture availability. This is presumably saturating. It may even appear unimodal, but where it does, this is probably because other factors associated with very high precipitation depress growth: these are likely increased snowpack, which shortens effective growing season, or decreased solar radiation during early parts of the growing season. 

# This likely saturating response shape presumably varies among genotypes and even among phenotypes for the same genotype (or even individual over time). 

# The hypotheses this study seeks to address can all be framed in terms of this response shape and its relationship to patterns of moisture availability across the region. 
# 1) As a species nears its range edge, we expect an increase in sensitivity. This prediction relies on the assumtion that there is a relatively consistent response function for each tree species and that this response function steepens toward lower levels of moisture availability. In drier regions, the trees express greater sensitivity because they are exposed to lower rainfall. If these assumptions hold roughly true, we would expect species approaching their range limit to exhibit a more rapid increase in sensitivity, because their (species-level) response function drops more sharply as moisture declines. 
...
ggplot(d_all, aes(ppt, raw_width, color=species))    + theme_bw()  + geom_smooth(method=loess, se=T, fullrange=FALSE)  +  theme(legend.position="none") + facet_wrap(~cluster.y)
